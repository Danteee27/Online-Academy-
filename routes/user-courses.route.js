...
router.get("/category/:id", async function (req, res) {
  ...
  if (field === null) return res.redirect("/");
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (tempTeacher !== null) list[i].instructor = tempTeacher.teacherName;
  }
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (list[i].promotion != 0) list[i].hasPromotion = true;
    list[i].star = star;
    let now = new Date();
...
router.get("/detail", async function (req, res) {
  ...
  if (
    ...
    typeof res.locals.authUser !== "undefined"
  ) {
  ...
  if (teacher !== null) course.instructor = teacher.teacherName;
  if (+course.completed === 0) course.completed = false;
  else course.completed = true;

  const catName = cat.catName;
  const lecture = await lectureService.findAllByCourseIDWithoutHidden(courseID);
  if (lecture.length > 0) lecture[0].isPreview = true;
  const recommendList = await courseService.find5BestSellerCoursesByCatID(
    courseID,
    catID
  );
  ...
  const feedbackList = await feedbackService.findByCourseIDWithLimit(
    courseID,
    4
  );
  ...
  for (let i = 0; i < feedbackList.length; i++) {
    const star = [];
    for (let j = 1; j <= 5; j++) {
      if (j <= +feedbackList[i].rating) star.push(true);
  ...
  const userTeacher = await teachersService.findByUserId(userID);
  let isOwner = false;
  if (userTeacher !== null) {
    isOwner = teacher.teacherID === userTeacher.teacherID;
  }
...
router.post("/buy-now", redirecting, async function (req, res) {
  ...
  if (course.student_num === null) course.student_num = 0;
  if (course.weekStudentNum === null) course.weekStudentNum = 0;
...
router.post("/moreFB", async function (req, res) {
  ...
  var list = await feedbackService.findByCourseIDWithLimitOffset(
    courseID,
    limit,
    offset
  );
  for (let i = 0; i < list.length; i++) {
    const star = [];
    for (let j = 1; j <= 5; j++) {
      if (j <= +list[i].rating) star.push(true);
      else star.push(false);
    }
    list[i].star = star;
...
router.post("/user-feedback", async function (req, res) {
  ...
  let totalRating = 0;
  for (let i = 0; i < fbList.length; i++) totalRating += +fbList[i].rating;
