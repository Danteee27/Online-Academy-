...
export default {
  ...
  async updateStudentNum(courseID) {
    ...
    return await db("courses")
      .where("courseID", courseID)
      .update("student_num", student_num);
  },

  updateRatingNum(courseID, rating_num) {
    return db("courses")
      .where("courseID", courseID)
      .update("rating_num", rating_num);
  },
  ...
  async checkCompleted(id) {
    ...
    const course = await db("courses").where("courseID", id);
    if (list.length === course[0].lec_num) {
  ...
  async fulltextSearch(searchStr, limit, offset, catID) {
    var sql =
      "select * from courses where match(courseName, tinydes) against ('" +
      searchStr +
    ...
    if (catID != 0) {
      sql =
        "select * from courses where catID = " +
        catID +
        " and match(courseName, tinydes) against ('" +
        searchStr +
        "' IN NATURAL LANGUAGE MODE) limit " +
        limit +
        " offset " +
    ...
    if (list[0].length === 0) return null;
    return list[0];
  },
  ...
  async fulltextSearchHighestRated(searchStr, limit, offset, catID) {
    var sql =
      "select * from courses where match(courseName, tinydes) against ('" +
      searchStr +
      "' IN NATURAL LANGUAGE MODE) order by rating desc limit " +
      limit +
      " offset " +
      offset;
    if (catID != 0) {
  ...
  async fulltextSearchNewest(searchStr, limit, offset, catID) {
    var sql =
      "select * from courses where match(courseName, tinydes) against ('" +
      searchStr +
      "' IN NATURAL LANGUAGE MODE) order by courses.update desc limit " +
      limit +
      " offset " +
      offset;
    if (catID != 0) {
  ...
  async fulltextSearchPrice(searchStr, limit, offset, type, catID) {
    var sql =
      "select * from courses where match(courseName, tinydes) against ('" +
      searchStr +
      "' IN NATURAL LANGUAGE MODE) order by price " +
      type +
      " limit " +
      limit +
      " offset " +
